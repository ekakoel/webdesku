<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-3">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Dokumen/Laporan APBDes</h2>
            <a href="{{ route('admin.village-apbdes-documents.create') }}" class="inline-flex items-center rounded-md bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800">Tambah Dokumen</a>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="GET" action="{{ route('admin.village-apbdes-documents.index') }}" class="grid gap-3 md:grid-cols-[1fr_220px_auto_auto]">
                    ...
                    <a href="{{ route('admin.village-apbdes-documents.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 text-center">Reset</a>
                </form>
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white">
                            @forelse ($items as $row)
                                <tr>
                                    ...
                                    <td class="px-3 py-2">
                                        <a href="{{ route('admin.village-apbdes-documents.edit', $row) }}" class="text-blue-700 hover:underline">Ubah</a>
                                        <form action="{{ route('admin.village-apbdes-documents.destroy', $row) }}" method="POST" class="inline-block ml-3" onsubmit="return confirm('Hapus dokumen APBDes ini?')">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                                <tr><td colspan="5" class="px-3 py-6 text-center text-gray-500">Belum ada dokumen/laporan APBDes.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
