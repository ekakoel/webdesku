...
class VillageHamletController extends Controller
{
    ...
    public function store(StoreVillageHamletRequest $request): RedirectResponse
    {
        ...
        if (! $village) return back()->withErrors(['name' => 'Data desa belum tersedia.'])->withInput();
    ...
    {
        if ($villageHamlet->householdWelfares()->exists()) return back()->withErrors(['name' => 'Banjar masih digunakan oleh data kesejahteraan KK dan tidak dapat dihapus.']);
